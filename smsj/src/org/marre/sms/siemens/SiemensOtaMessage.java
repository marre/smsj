...
public class SiemensOtaMessage extends SmsAbstractMessage
{
		...
		protected long referenceId = 0;
		protected int dataSize = 0;
		protected int numberOfPackets = 0;
		protected byte[] content = null;
	
		/**
		 * Creates an SMS containing a Bitmap or a Ringtone
		 *
		 * @param theMessage The content in bytes
		 */
		...
		{
				this.setDataCodingScheme((byte) 0xf5);
		...
		private void init()
		{
			this.referenceId = (new Random()).nextLong();
			this.dataSize = 140 - 22 - name.length() - type.length();
			this.numberOfPackets = (int)Math.ceil((float)content.length / (float)dataSize);
		}
		...
		{
			byte[] header = new byte[22 + name.length() + type.length()];
			...
			// ReferenceId			
			header[8] = (byte) (referenceId & 0xff);
			header[9] = (byte) ((referenceId >> 8) & 0xff);
			header[10] = (byte) ((referenceId >> 16) & 0xff);
			header[11] = (byte) ((referenceId >> 24) & 0xff);
			...
			// NumberOfPackets
			header[14] = (byte) (numberOfPackets & 0xff);
			header[15] = (byte) ((numberOfPackets >> 8) & 0xff);
			
			// ObjectSize
			header[16] = (byte) (content.length & 0xff);
			header[17] = (byte) ((content.length >> 8) & 0xff);
			header[18] = (byte) ((content.length >> 16) & 0xff);
			header[19] = (byte) ((content.length >> 24) & 0xff);
			
			// ObjectType (Pascal String)
			header[20] = (byte) (type.length() & 0xff);
			header[21] = (byte) (type.charAt(0) & 0xff);
			header[22] = (byte) (type.charAt(1) & 0xff);
			header[23] = (byte) (type.charAt(2) & 0xff);
			
			// ObjectName (Pascal String)
			header[24] = (byte) (name.length() & 0xff);
			for (int i = 0; i < name.length(); i++) {
				header[25 + i] = (byte) ((name.charAt(i)) & 0xff);
			}
		...
		{
				SmsPdu smsPdus[] = new SmsPdu[numberOfPackets];				
				...
				for (int i = 0; i < numberOfPackets; i++) {					
					...
					int offset = dataSize * i;					
					
					// Handle last SMS
					if (i == (numberOfPackets - 1)) {
						dataSize = content.length % dataSize;
					}
										
					System.arraycopy(content, offset, pdu, header.length, dataSize);
		...
		public static void main(String[] args) {
			...
			for (int i = 0; i < pdus.length; i++){					
				System.out.println("UD     : " + StringUtil.bytesToHexString(pdus[i].getUserData()));
			}
		}
